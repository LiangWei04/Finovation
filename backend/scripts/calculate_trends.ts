...
function halfYearFromDate(dateText: string): string {
  const date = new Date(`${dateText}T00:00:00Z`);
  const half = date.getUTCMonth() < 6 ? 1 : 2;
  return `${date.getUTCFullYear()}-H${half}`;
}

function halfYearBounds(period: string): { start: string; end: string } {
...
function buildTrendPoint(company: Company, period: string, signals: EsgSignal[]): EsgTrendPoint {
  const bounds = halfYearBounds(period);
...
async function main(): Promise<void> {
  ...
  for (const signal of signals) {
    ...
    const period = halfYearFromDate(signal.published_date);
  ...
  console.log(`Calculated ${trendPoints.length} half-year evidence trend points. Output: ${TRENDS_PATH}`);
}
