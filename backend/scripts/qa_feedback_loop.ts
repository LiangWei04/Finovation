...
function expectedTrendPeriod(dateText: string): string {
...
async function main(): Promise<void> {
  ...
  for (const signal of signals) {
    ...
    if (/404|page not found|access denied|just a moment/i.test(signal.evidence_summary)) {
      issue(issues, "error", `${signal.signal_id} has an error-page title in its evidence summary.`);
    }
  }
  ...
  const signalsByTrendKey = groupBy(
    ...
    (signal) => `${signal.company_id}:${expectedTrendPeriod(signal.published_date!)}`,
  );
  ...
  for (const source of rawSources) {
    ...
    if (source.extraction_status === "fallback_seeded" && /404|page not found|access denied|just a moment/i.test(source.title)) {
      issue(issues, "error", `${source.source_id} fallback source kept an error-page title: ${source.title}.`);
    }
    if (/^\s*404\s*$|404 - page not found|page not found|access denied|just a moment/i.test(source.title)) {
      issue(issues, "error", `${source.source_id} has an error-page title: ${source.title}.`);
    }
    const metadataTitle = source.firecrawl_metadata.title;
    if (typeof metadataTitle === "string" && /^\s*404\s*$|404 - page not found|page not found|access denied|just a moment/i.test(metadataTitle)) {
      issue(issues, "error", `${source.source_id} has an error-page metadata title: ${metadataTitle}.`);
    }
  }
